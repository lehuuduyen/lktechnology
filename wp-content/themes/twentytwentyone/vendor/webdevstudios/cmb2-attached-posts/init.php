...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		if ( $this->field->options( 'filter_boxes' ) ) {
			$filter_boxes = '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( '検索', 'cmb' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>';
		}
	...
	public function list_item( $object, $li_class, $icon_class = 'dashicons-plus' ) {
		// Build our list item
		printf(
			'<li data-id="%1$d" class="%2$s" target="_blank">%3$s<a target="_blank" title="' . __( 'Edit' ) . '" href="%4$s">%5$s</a>%6$s<span class="dashicons %7$s add-remove"></span></li>',
			$this->get_id( $object ),
			$li_class,
	...
	public function get_title( $object ) {
		if($this->field->options( 'query_users' )){
